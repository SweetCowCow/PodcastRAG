...
> 最後更新：2026-05-03（admin-llm-step-config archive 後）
...
| **T3** | 每集 AI 摘要（`episode-ai-summary` change 已 propose + parked，32 tasks）| 🆕 下一個要做。前置：admin 在 prod 把 `ai_steps.summary` 設好（建議 `gpt-5-mini` via Zeabur AI Hub） |
