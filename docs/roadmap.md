...
| **O3 → db-backup** | ⭐ **superseded by `db-backup` change（implementation in progress 2026-05-07）** | 24h RPO / 30min RTO。每日 03:00 UTC pg_dump → age 加密 → Cloudflare R2 離站。月度 GHA 自動還原驗證。code 已寫完，等首次 scheduled beat run + GHA monthly verify 驗證後 archive。詳見 `docs/disaster-recovery.md` |
