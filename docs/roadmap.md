...
- 🔥 **`langfuse-self-host-evaluation`**（**優先**，4.4 結果驅動）— Cloud SDK roundtrip 對 span-heavy retrieval workload 加 +5-6s。自架 on Linode SIN g6-standard-2 LAN 內 RPC < 50ms。起手 `/spectra-discuss langfuse-self-host` 評估 VPS 容量 + ClickHouse 磁碟成長 + 遷移路徑
- 🔄 **`eval-runner-eval-context-plumbing`** — 讓 (item_id, turn_idx) 進 PG eval_traces 開 SQL RCA flow（design Decision 5 canonical）。目前 prompt_fingerprint_diff.py 走 `?debug_trace=true` inline 萃取、functional 但跨 run 對比走 Cloud UI / 用 export 不順
