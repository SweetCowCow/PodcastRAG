...
> 最後更新：2026-06-05（EQ3a 衍生的「詞彙失配」track 完成：`lexical-mismatch-query-rewrite-bakeoff` + `hyde-retrieval-landing` ✅ 皆 archive。HyDE prod A/B 是雙面刃→維持 flag off，衍生 conditional-HyDE follow-up。**b20 的詞彙失配根因已被此 track 涵蓋，EQ3b 原始目的（b20 RCA）失去主要必要性**。剩餘 retrieval 待辦＝b22 獨立 change、b23 獨立 change、conditional-HyDE follow-up（見下方 EQ3a-followup 段）。執行佇列為權威排序，下方 Phase A–F 表為細節參考。）
...
| **EQ3a-f3** | HyDE 有條件啟用 | `hyde-conditional-activation`（暫名） | ⬜ | EQ3a-f2 | 只在偵測詞彙失配訊號才開 HyDE（避免傷 lexical 已對齊查詢）。觸發訊號待設計（query↔top-k 詞面重疊 / lexical 分數門檻 / 兩階段補召）。落地基礎已備 |
| **EQ3a-f4** | b22 routing 救援（獨立 change） | `b22-routing-distributed-evidence`（暫名） | ⬜ | EQ3a | agent 走 get_show_overview/search_across_episodes 沒進 voyage path + distributed-evidence miss。診斷工具 `audit_voyage_pipeline.py` 可複用 |
| **EQ3a-f5** | b23 narrative retrieval（獨立 change） | `b23-transcript-aware-narrative`（暫名） | ⬜ | EQ3a | full-show hybrid+voyage 繞過 title/desc topic-prefilter（EP107 transcript-buried，ts_rank 排不進前 20）。見 `reference_topic_prefilter_transcript_buried_limit` |
