...
- ⛔ **`lexical-bm25-replace-ts_rank`**（原候選 C）— 暫不開。也走 weighting 路線、同 IDF 盲點風險，要動之前必須先有 evaluation framework 升級觀察到 IDF 前提是否成立
- 🆕 **`eval-framework-upgrade`**（promoted to next，原候選 E）— 開新 change：(a) per-question tool-call trace 落地（args + result chunks + search query）(b) episode-scoped retrieval probe tool（retrieval PR 必跑）(c) span-level metric (Ragas-style) (d) prompt change fingerprint diff (e) Langfuse / OpenLLMetry / Arize Phoenix tracing。User 2026-05-28 拍板「品質指標的部分應該要納入框架來做」。起手 `/spectra-discuss eval-framework-upgrade` 拆刀法 + 跟既有 `rag-eval-runner` / `rag-eval-judge` 共存策略
