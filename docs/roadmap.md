...
- 🔥 **`eval-runner-eval-context-plumbing`**（升優先序）— 讓 (item_id, turn_idx) 進 PG eval_traces 開 SQL RCA flow（design Decision 5 canonical）。目前 prompt_fingerprint_diff.py 走 `?debug_trace=true` inline 萃取、functional 但跨 run 對比走 Cloud UI / 用 export 不順
