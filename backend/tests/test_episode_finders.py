...
# ---------------------------------------------------------------------------
# voyage-rerank-tune-b22-b23 Stage B: topic prefilter relevance cap
# ---------------------------------------------------------------------------

@pytest.mark.asyncio
async def test_find_by_topic_caps_episodes_by_relevance(monkeypatch):
    """Stage B: the topic SQL MUST rank candidates by `ts_rank` relevance and
    apply `LIMIT :max_eps` when `topic_prefilter_max_episodes` > 0, so a
    host-name token can't balloon the candidate pool (b23: 64 episodes) and
    dilute cross-episode GT before retrieve_hybrid runs."""
    tokenizer.reset_for_tests()
    monkeypatch.setattr(episode_finders.settings, "topic_prefilter_max_episodes", 7)
    db = _mock_db_with_rows([])
    await episode_finders.find_episodes_by_topic(db, uuid.uuid4(), ["歌單"])
    sql_str = str(db.execute.call_args[0][0])
    params = db.execute.call_args[0][1]
    assert "ts_rank(" in sql_str
    assert "ORDER BY topic_relevance DESC" in sql_str
    assert "LIMIT :max_eps" in sql_str
    assert params["max_eps"] == 7
    # WHERE-clause match predicates preserved (still searches title + desc)
    assert "title_tsvector @@ to_tsquery('simple', :tsquery_text)" in sql_str
    assert "d.text_tsvector @@ to_tsquery('simple', :tsquery_text)" in sql_str


@pytest.mark.asyncio
async def test_find_by_topic_cap_disabled_when_setting_nonpositive(monkeypatch):
    """Env rollback: `TOPIC_PREFILTER_MAX_EPISODES=0` (<=0) disables the cap —
    no LIMIT clause, no `max_eps` param — restoring unbounded behaviour."""
    tokenizer.reset_for_tests()
    monkeypatch.setattr(episode_finders.settings, "topic_prefilter_max_episodes", 0)
    db = _mock_db_with_rows([])
    await episode_finders.find_episodes_by_topic(db, uuid.uuid4(), ["歌單"])
    sql_str = str(db.execute.call_args[0][0])
    params = db.execute.call_args[0][1]
    assert "LIMIT" not in sql_str
    assert "max_eps" not in params
    # relevance ordering still applied even when uncapped
    assert "ORDER BY topic_relevance DESC" in sql_str
