...
- [ ] 6.1 跑既有 chat agent 相關單元測試（`test_chat_agent_*.py`）確認無回歸；部署後（flag 預設 on）用 `backend/scripts/b23_prod_smoke.sh` 對 prod 打 b23 題，確認 agent 第一個工具呼叫為 `search_with_topic_prefilter`（debug_trace `tool_calls[0].name`）、回應引用到 EP107 的初遇 / 合作 GT 段。驗證：既有測試全綠；prod smoke trace 顯示第一工具 = search_with_topic_prefilter 且 citations 含 EP107。
