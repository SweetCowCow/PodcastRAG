...
- [ ] 8.5 在瀏覽器（Zeabur prod）驗證 issue 2 + 3：頂端按鈕顯示「新增節目」、點開後面板標題顯示「新增節目轉錄排程」；任意 schedule != null 卡片的「立刻執行轉錄」按鈕文字正確、點擊後 alert 行為照舊不變 — 對應「ScheduleTab page header uses Add Show language」與「Single-show 立刻執行轉錄 still skips the confirm modal」
