...
- [ ] 1.1 在 `backend/scripts/agentic_bakeoff/tools/` 下定義 9 個 tool 的統一 input/output schema（Pydantic BaseModel），其中 8 個 stub 回 fixture data、`search_across_episodes` 接 `app.services.rag.retrieve_hybrid`、`pin_episode` / `unpin_episode` 真寫 Redis L1 state（落實 design.md「9 Tool Spec」表）
