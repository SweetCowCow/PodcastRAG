...
2. If `len(candidates) > 0`, call `rag.retrieve_hybrid(query, episode_id_filter=[ep.episode_id for ep in candidates], k=20)` to obtain an expanded candidate chunk pool `top_n_chunks`. If `len(candidates) == 0`, fall back to `rag.retrieve_hybrid(query, k=k)` without filter and skip rerank (return directly with `rerank_applied=false`).
...
- `rerank_input_count`: integer, the number of chunks sent to the rerank stage (`len(top_n_chunks)`, which is at most 20 and may be smaller when the prefilter pool has fewer than 20 chunks). When the empty-candidate fallback path was taken, this field SHALL be `0`.
...
- **THEN** the underlying `rag.retrieve_hybrid` call SHALL pass `episode_id_filter=[EP143_uuid, EP107_uuid]` and `k=20`
...
#### Scenario: Rerank reorders top-20 candidates and returns top-k

- **GIVEN** prefilter path produced 20 candidate chunks `C1..C20` in RRF order
...
- **AND** the envelope's `rerank_input_count` SHALL equal `20`

#### Scenario: Rerank failure falls back to original RRF order

- **GIVEN** prefilter path produced 20 candidate chunks
- **WHEN** the LLM rerank call times out (exceeds 1.5s), returns non-2xx, returns malformed JSON, or returns `ranked_chunk_ids` whose entries are all absent from the candidate set
- **THEN** the tool SHALL return the first `k` chunks of the original RRF order
- **AND** the envelope's `rerank_applied` SHALL be `false`
- **AND** the envelope's `rerank_input_count` SHALL still report the actual number of chunks sent to rerank (e.g., `20`)
...
- **GIVEN** prefilter path produced 20 candidate chunks `C1..C20`
