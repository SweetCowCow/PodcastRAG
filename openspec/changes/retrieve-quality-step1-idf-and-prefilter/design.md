...
## Postmortem (added 2026-05-28 after both layers reverted)

### Layer A Postmortem — show-wide IDF on podcast transcripts doesn't work

**What was shipped**: `transcript_token_freq` (per-show) + bucketed `ts_rank` sum in `_TRANSCRIPT_RRF_SQL` (already a *revised* Decision A2 — original `ts_rank_cd` weight array plan was scrapped at apply time after PG semantic verify, see Decision A2 修訂版 above).

**Eval result (chat baseline, 34 records, commit aea7590)**:
- chunk_recall_grouped 0.482 → 0.382 (-0.100)
- factual_correctness 0.892 → 0.831 (-0.061)
- 7 cases regressed (b08, b12, b18, b20, b27, b28, b29, b30, mt04)

**Worst case b18** (EP44 伴手禮, chunk_recall 1.0→0.0, factual 1.0→0.4):
- Same 3-tool path as baseline (`find_episode_by_ref` + `search_within_episode` + 1 more)
- OLD answer correctly cited "鯛魚燒" example chunks
- NEW answer: "查詢結果無法明確說明使用了哪些具體例子" — GT chunks dropped out of top-K

**Root cause**: Show-wide IDF labels query tokens like 「伴手禮」/「送禮」as bucket A (rare in 145-episode show corpus). When the agent's `search_within_episode` is then restricted to EP44's ~30 chunks (which is *the* 伴手禮 episode), those tokens are now **everywhere within the episode**. Bucket A's 1.0 weight amplifies the lexical signal of chunks containing those topic words, but those are setup / discussion chunks — NOT the answer chunks (which use phrasing like 「現吃好吃」「鯛魚燒」). Bucketed weighting promotes topic-related-but-not-answer chunks above the actual answer.

This is exactly the failure mode that the chat agent's existing SYSTEM_PROMPT already warns against ("相關 ≠ 答案"). IDF weighting on the retrieval layer amplifies that confusion instead of helping.

**The fundamental orthogonality flaw**:

| IDF 假設 | Podcast 實際 |
|---|---|
| 罕見 token = answer signal | 罕見 token 是**整集主題**，在 answer episode 內反而常見 |
| Show-wide IDF 能區分 chunk 信號 | Show-wide IDF 對 **episode 內部** chunks 沒區分力 |
| 常見 token = noise | answer chunk 常用一般詞彙描述 |

**Why the b14 prod DB probe didn't catch this**: The probe (task 3.2) measured rank within the LEXICAL POOL across all transcript chunks of the show (10K+ chunks). It showed `405a7b8c` at #1 in both legacy and bucketed — looked fine. But the chat agent's actual `search_within_episode` call passes `episode_id_filter`, which restricts the pool to ~30 chunks (just EP44). Inside that tiny pool, bucketed weighting reorders chunks differently than the cross-show probe predicted. The probe was a false positive validation.

**Lesson**: When validating retrieval changes against real chat behavior, always include episode-scoped probes (with `episode_id_filter`), not just show-wide probes. The two pool sizes give wildly different signals.

### Layer B Postmortem — prompt nudges are not orthogonal to retrieval

**What was shipped**: `prompts.py` SYSTEM_PROMPT prepended with "Episode Reference Resolution" section (mandate `find_episode_by_ref` + `search_within_episode` as first two tool calls for EP-ref queries) + `tools.py` extended `search_within_episode` description with priority language.

**Eval result (chat baseline, 34 records, commit 0e38b16)**:
- chunk_recall_grouped 0.482 → 0.340 (-0.142, **worse than A**)
- factual_correctness 0.892 → 0.875 (-0.017, near-neutral)
- 6 cases regressed (b08, b18, b20, b23, b29, mt04)
- **answer_contradict_check** 0 → 1 ✅ (b14 finally caught the "故意不選振奮歌" reversal — small win)
- b14 factual 0.5 → 0.8 ✅

**Worst case mt04** (EP19 動漫歌單 multi-turn, cr 0.375→0, fc 0.8→0.4):
- Turn 0 OLD: 3 tools / Turn 0 NEW: 1 tool (B's prompt encouraged dispatching once early)
- Turn 1 OLD: 1 tool / Turn 1 NEW: 2 tools (NEW agent had to call extra tool to compensate for missing context)
- Turn 2 OLD: 1 tool / Turn 2 NEW: 4 tools (further compensation)
- Net effect: agent's path through tools got reshuffled in ways that lost citation chunks

**Root cause**: We hypothesised that B was "pure prompt dispatch nudge, doesn't touch retrieval". In practice:
1. New `Episode Reference Resolution` section changed agent's overall query formulation behavior (not just dispatch order)
2. `search_within_episode` takes a `query: str` argument — when agent re-phrases the search query (e.g., "送禮糾結" vs "伴手禮 現吃好吃"), the downstream `_build_ts_query` produces different tokens
3. Different tsquery tokens → different ts_rank result → different top-K → different GT hits
4. Even with identical retrieval SQL, the **prompt change leaves a fingerprint on retrieval input**

**Lesson**: Prompt changes have non-local effects via tool argument generation. The "low-risk because it doesn't touch SQL" framing was wrong. This extends the existing memory `feedback_prompt_saturation_more_is_less.md` ("加更多 example 反而 regress") — it's not just example density, **any structural new prompt section has retrieval side effects**.

### Combined lesson for next change proposal

**Before shipping ANY retrieval / agent change, we need observability that captures**:

1. **Per-question tool-call trace**: full sequence of (tool_name, tool_args, tool_result_chunks) per turn, persisted with the eval result. Current `agent_responses_meta` only stores `tool_calls_n`, not the args/results.
2. **Search query string per `search_*` call**: so we can diff "what the agent actually queried" pre/post prompt change.
3. **Episode-scoped retrieval probe**: validate with `episode_id_filter` set, not just show-wide pool, before declaring "rank improved".
4. **Span-level metric (Ragas-style)**: per-chunk relevance / per-question answer faithfulness, not just aggregate `chunk_recall` / `factual_correctness`.

These are exactly what the parked **evaluation framework upgrade** candidate is supposed to deliver. Promoting it from "Phase 2 backup" to **Phase 1 must-have before next retrieval change**.
