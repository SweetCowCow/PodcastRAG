...
The `HomePage` SHALL include a section labelled 收錄節目 (`zh`) / Collected Shows (`en`) below the mode trio. This section SHALL render one card per show returned by `GET /shows` using a `<ShowCard>` component. Each card SHALL display: cover image (with fallback colored icon when `image_url` is missing), show title, language label, description rendered as **plain text only** (all HTML tags MUST be stripped, `<br>` `<br/>` `<br />` MUST be converted to newline characters, common HTML entities such as `&amp; &lt; &gt; &quot; &#39; &nbsp;` MUST be decoded to their character form, and clamped to 3 lines), total episode count, transcribed episode count, transcription progress bar with percentage, RSS feed URL, and an `進入節目` (`zh`) / `Open Show` (`en`) link. Clicking anywhere on a card SHALL navigate to the show's QueryPage. The grid SHALL use `repeat(auto-fill, minmax(min(320px, 100%), 1fr))` on desktop and `minmax(0, 1fr)` (single column) on mobile, so that grid track sizing SHALL NOT exceed the container width regardless of card content. The RSS feed URL row SHALL render on a single line truncated with an ellipsis (`minWidth: 0` on the flex/grid item plus `textOverflow: ellipsis`), and no unbroken string inside a card (URL or otherwise) SHALL expand the card beyond the grid container width.
...
#### Scenario: Long unbroken RSS URL does not expand the card on mobile

- **GIVEN** a 375 px-wide mobile viewport and a show whose RSS feed URL is an unbroken string longer than the viewport width (e.g., a long omnycontent playlist URL)
- **WHEN** the show grid renders
- **THEN** every show card's rendered width SHALL be less than or equal to the grid container width
- **AND** the RSS URL SHALL render on a single line truncated with an ellipsis
- **AND** the `進入節目` / `Open Show` link SHALL be inside the visible viewport without horizontal scrolling

#### Scenario: Grid stays within container on desktop with long content

- **GIVEN** a 1280 px-wide desktop viewport and the same long-URL show
- **WHEN** the show grid renders
- **THEN** no grid track SHALL exceed the grid container width and the visual multi-column layout SHALL be unchanged from the pre-change desktop rendering
