...
PodcastRAG 目前完全沒有真實 auth：前端 `src/App.jsx:22` 用 `if (user === 'admin' && pass === 'admin999')` 判斷登入，後端 `/admin/*` / `/queue/*` / `/schedules/*` / `/settings/*` / `POST /shows` / `DELETE /shows/{id}` 全部裸露。前後端跨 zeabur 子網域（`podcastrag.zeabur.app` ↔ `podcastrag-api.zeabur.app`）。LLM 走 Zeabur AI Hub 計費，query API 一旦對外公開，無 per-user cap 會被燒爆。
...
- 公開 repo 前用 `git filter-repo` 洗 history 中 `admin123` / `admin999`
...
**選擇**：env `ADMIN_EMAILS=ssweetcoww@gmail.com,foo@bar.com`（comma-separated）。Google callback 處理 user upsert 時：
...
**Phase 1 — Zeabur 部署**：
1. Zeabur 後端 service 補 env：`GOOGLE_CLIENT_ID` / `GOOGLE_CLIENT_SECRET` / `SESSION_SECRET` / `ADMIN_EMAILS=ssweetcoww@gmail.com` / `FRONTEND_ORIGIN=https://podcastrag.zeabur.app`
...
- 前端 hardcoded admin999 在 phase 1 中段才移除；若 phase 1 失敗，revert 該 commit 即可恢復原 modal 行為（因為 `/admin/*` gate 同時 revert，dependency 沒上線就無影響）
