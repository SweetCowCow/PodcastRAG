...
目前後台帳密 `admin / ***REDACTED***` 寫死在前端 `src/App.jsx:22`，任何看到原始碼的人都能登入；後端 `/admin/*` endpoints 完全沒有 auth gate，知道 URL 就能直接呼叫。這個安全債阻擋 GitHub repo 公開、阻擋分享給外部試用者，且使用者開放外部試用後 LLM 額度沒有 per-user 上限會被燒爆。本 change 建立正規多人帳號驗證系統，導入 Google SSO + RBAC + 查詢次數 quota，為後續公開做準備。
...
- **前端**：移除 `AdminLogin` 內 hardcoded `***REDACTED***` 判斷 → 改打 `/auth/google/start` 跳轉；新增 `useCurrentUser` hook 從 `/me` 拉資料；右上角顯示登入狀態 + 剩餘 quota；後台新增「使用者管理」tab（欄位：Avatar / Name / Email / Role / Status / Provider / Created / Last login / Total queries / Quota remaining / Notes / 操作）
...
- **公開 repo 前清理**：用 `git filter-repo` 從 history 移除 `***REDACTED***` / `***REDACTED***` 字串
...
- Affected code:
  ...
  - Modified:
    ...
    - src/App.jsx（移除 hardcoded ***REDACTED***、改用 AuthContext、admin 進入前驗 role）
  ...
  - Removed:
    - 無檔案級刪除；僅移除 `src/App.jsx` 內 hardcoded admin/***REDACTED*** 判斷邏輯
