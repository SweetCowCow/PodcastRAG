...
- `quota_digest` / `eval_reminder` / `db_backup` / `tokenizer_reload` 的 `apply_async` 呼叫點是否要顯式加 `queue="control"` 還是靠 task_routes 自動分流？傾向 task_routes 集中管理，避免散落程式碼
- worker idempotency check 的 5 min window 是否需要 env 可調？傾向先寫死，未來如要調再加
