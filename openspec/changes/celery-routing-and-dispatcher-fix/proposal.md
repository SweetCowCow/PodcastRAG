...
- **worker task 進場 idempotency check**：若 row 已 `running` 且 `started_at` 在 5 min 內 → 直接 ack 不重跑（防 dispatcher 連發 race）
...
- Affected code:
  ...
  - New:
    ...
    - `backend/tests/test_dispatcher_idempotency.py`（worker idempotency check 驗證）
